...
class _ScrubberWidgetState extends State<ScrubberWidget>
  ...
  // Computed from bookData
  List<String> _bookIds = [];
  List<String> _titles = [];
  List<int> _pageCounts = [];
  ...
  void initState() {
    ...
    _computeFromBookData(widget.bookData);
  ...
  void didUpdateWidget(ScrubberWidget oldWidget) {
    ...
    if (!identical(widget.bookData, oldWidget.bookData)) {
      _computeFromBookData(widget.bookData);
    }
  ...
  void _computeFromBookData(Map<String, ({int pageCount, String title})> map) {
    ...
    _pageStarts = [];
    int cumulative = 0;
    for (final pc in _pageCounts) {
      _pageStarts.add(cumulative);
      cumulative += pc;
    }
  ...
  ({String id, int localPage}) _globalToBook(int globalIndex) {
    if (_totalPages == 0) return (id: '', localPage: 0);
    globalIndex = globalIndex.clamp(0, _totalPages - 1);
    int lo = 0, hi = _bookIds.length - 1;
    while (lo < hi) {
      final mid = (lo + hi + 1) ~/ 2;
    ...
    return (id: _bookIds[lo], localPage: globalIndex - _pageStarts[lo]);
  }
  ...
  void _onTick(Duration elapsed) {
    if (!_active || _velocityMag == 0 || _totalPages == 0) return;
  ...
  void _deactivate() {
    ...
    if (_active && _totalPages > 0) {
      final info = _globalToBook(_idx.round());
      widget.onNavigate(info.id, info.localPage);
    }
  ...
  Widget _buildStripContents() {
    ...
    for (int p = _winStart.floor(); p <= winEnd; p++) {
      final pi = _globalToBook(p);
      final idx = _idToIndex[pi.id] ?? 0;
    ...
    final currentBookId = _globalToBook(_idx.round()).id;
